import { Product, Campaign, UnitEconomics, SimulationResult, RiskLevel, BatchSimulationSummary } from '../types/index.js';
import { getCategoryCommission } from './commissionEngine.js';
import { calculateShipping } from './shippingEngine.js';
import { calculateTaxes } from './taxEngine.js';
...
export function calculateUnitEconomics(
  ...
  const { commissionRate, serviceFee } = getCategoryCommission(
    product.category,
    product.marketplace,
  ...
  const shippingInfo = calculateShipping(product.desi, discountedPrice, product.marketplace);
  ...
  const taxBreakdown = calculateTaxes(
    discountedPrice,
    product.costPrice,
    product.saleVatRate || 20,
    product.costVatRate || 20,
    commissionAmount,
    shippingFee
  );
  ...
  const netPayoutFromMarketplace = Number(
    (discountedPrice - commissionAmount - serviceFee - shippingFee - taxBreakdown.stopajWithholding).toFixed(2)
  );
  ...
  const netProfit = Number(
    (netPayoutFromMarketplace - product.costPrice - taxBreakdown.netVatPayable).toFixed(2)
  );
 ...
 */
export function simulateProductCampaign(
  product: Product,
  campaign: Campaign,
  customDiscountPercent?: number
): SimulationResult {
  ...
  let riskLevel: RiskLevel = 'SAFE_PROFITABLE';
 ...
 */
export function simulateBatchCampaign(
  products: Product[],
  campaign: Campaign
