...
export interface CategoryRateInfo {
  category: string;
  marketplace: MarketplaceType;
  commissionRate: number; // Yüzde (%)
  serviceFee: number;     // Sipariş / Ürün Başına Sabit Hizmet Bedeli (TL)
}

// Türkiye Pazaryerleri Standart Kategori Komisyon Oranları (2024-2026)
export const DEFAULT_COMMISSION_RATES: CategoryRateInfo[] = [
  // Trendyol
...
export function getCategoryCommission(category: string, marketplace: MarketplaceType, customRate?: number): { commissionRate: number; serviceFee: number } {
  if (customRate !== undefined && customRate > 0) {
    const defaultEntry = DEFAULT_COMMISSION_RATES.find(r => r.marketplace === marketplace && r.category === category);
    return {
  ...
  const match = DEFAULT_COMMISSION_RATES.find(
    r => r.marketplace === marketplace && r.category.toLowerCase() === category.toLowerCase()
  );
